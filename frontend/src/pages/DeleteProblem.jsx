...
import { Sun, Moon, Code, Search, Trash2, Check, AlertTriangle } from "lucide-react"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center">
        ...
        >
          <h2 className="text-3xl font-bold text-black dark:text-white mb-2">Problem Deleted Successfully!</h2>
          <p className="text-gray-600 dark:text-gray-400 mb-2">
            <strong>"{problemTitle}"</strong> has been permanently removed.
          </p>
  ...
  return (
    ...
    style={{ top: scrollTop, height: "100vh" }}>
      ...
      <div className="relative w-full max-w-md bg-white/95 dark:bg-black/95 backdrop-blur-xl rounded-2xl border border-gray-200 dark:border-gray-700 shadow-2xl p-6 transform transition-all duration-300 scale-100">
        ...
        {/* Title */}
        <h3 className="text-xl font-bold text-center text-black dark:text-white mb-2">Confirm Deletion</h3>

        {/* Message */}
        <div className="text-center text-gray-600 dark:text-gray-400 mb-6">
          <p className="mb-2">Are you sure you want to permanently delete:</p>
    ...
    <div className="absolute inset-0 -z-10 h-full w-full overflow-hidden bg-white dark:bg-black">
      ...
      <div className="absolute inset-0 opacity-[0.03] dark:opacity-[0.08]">
        {Array.from({ length: 25 }).map((_, i) => (
          <div
            key={i}
            className="absolute w-1 h-1 bg-red-500 dark:bg-red-400 rounded-full animate-pulse"
            style={{
      ...
      <div className="hidden dark:block absolute inset-0 overflow-hidden opacity-[0.08]">
        {Array.from({ length: 15 }).map((_, i) => (
          <div
            key={i}
            className="absolute text-red-400 text-xs font-mono animate-pulse"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
      ...
      <div className="hidden dark:block absolute inset-0 opacity-[0.03]">
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-red-900/5 to-transparent animate-pulse" />
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="absolute text-red-300 text-[8px] font-mono opacity-30"
            style={{
              left: `${10 + i * 12}%`,
              top: `${Math.random() * 80}%`,
      ...
      <div
        ...
        style={{
          backgroundImage: `
            radial-gradient(400px circle at var(--mouse-x) var(--mouse-y), rgba(239, 68, 68, 0.08), transparent 50%),
            radial-gradient(600px circle at var(--mouse-x) var(--mouse-y), rgba(220, 38, 38, 0.04), transparent 70%)
          `,
        }}
      />
      ...
      <div className="absolute inset-0 overflow-hidden opacity-[0.04] dark:opacity-[0.08]">
        {Array.from({ length: 12 }).map((_, i) => (
          <div
            key={i}
            className="absolute w-0.5 h-0.5 bg-red-400 dark:bg-red-300 rounded-full animate-bounce"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
  ...
  return (
    <div className="fire-particles">
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={`code-${i}`}
          className="absolute text-red-400 dark:text-red-400 text-xs font-mono opacity-20 animate-pulse"
          style={{
            left: `${Math.random() * 100}%`,
            top: `${Math.random() * 100}%`,
            animationDelay: `${Math.random() * 4}s`,
      ...
      <div className="relative z-10 p-4 rounded-xl bg-white/95 dark:bg-black/95 backdrop-blur-sm border border-gray-200 dark:border-gray-700 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 flex-1">
            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-gradient-to-br from-red-100 to-orange-100 dark:from-red-900/30 dark:to-orange-900/30 flex items-center justify-center text-xs font-semibold text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-600">
              {index + 1}
            </span>
...
export function DeleteProblem() {
  ...
  const { displayText, isComplete, showCursor } = useTypewriter("Delete Problems", 80)
  ...
  // --- FIX 3: Add this useEffect to control the scroll lock ---
  useEffect(() => {
    const isAnyModalVisible = isModalOpen || showSuccessModal;
    const element = containerRef.current;

    if (element) {
      if (isAnyModalVisible) {
        // When a modal is open, lock the container's scroll
        element.style.overflow = 'hidden';
      } else {
        // When all modals are closed, restore scroll
        element.style.overflow = '';
      }
    }
    
    // Cleanup function to ensure scroll is restored if you navigate away
    return () => {
      if (element) {
        element.style.overflow = '';
      }
    };
  }, [isModalOpen, showSuccessModal]); // Re-run whenever a modal's visibility changes
  ...
  return (
    ...
    <div ref={containerRef} className="min-h-screen relative text-black dark:text-white transition-colors duration-300">
        ...
        <div className="relative z-10 bg-white/95 dark:bg-black/95 backdrop-blur-xl rounded-2xl border border-gray-300 dark:border-gray-600 shadow-2xl p-6">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Manage and remove existing coding challenges from the platform.
            </p>
          </div>
              ...
              <div className="flex items-center gap-2 flex-1">
                <div className="relative w-full max-w-sm">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-3 py-2.5 rounded-xl bg-gray-50/80 dark:bg-gray-900/80 backdrop-blur-sm border border-gray-200 dark:border-gray-700 text-black dark:text-white placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500/20 dark:focus:ring-red-400/20 focus:border-red-500 dark:focus:border-red-400 transition-all duration-300 font-medium text-sm"
                  />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                {filteredProblems.length > 0 ? (
                  filteredProblems.map((problem, index) => (
                    <ProblemCard key={problem._id} problem={problem} index={index} onDelete={openDeleteModal} />
                  ))
                ) : (
